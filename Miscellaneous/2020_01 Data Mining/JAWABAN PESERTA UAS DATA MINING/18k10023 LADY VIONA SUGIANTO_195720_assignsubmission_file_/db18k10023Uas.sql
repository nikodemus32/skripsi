...
DELIMITER $$
CREATE PROCEDURE spHitLoop()
BEGIN
    DECLARE i ,vstop int default 0; 
    while i = 0 do
        CALL spData();
        CALL spIterasi();
        CALL spHitEntrop();
        CALL spHitG();
        CALL spNew();
        SELECT COUNT(*) INTO vstop from tblHitung 
        where gain = (select max(gain) from tblHitung) and flu != 0 and demam != 0; ;
        if vstop = 0 then 
          set i = 1;
        end if;
    end while;
