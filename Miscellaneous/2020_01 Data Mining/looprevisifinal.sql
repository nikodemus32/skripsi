...
load data local infile "dbC45Asli.csv"
into table tblC45
...
BEGIN
		...
		SELECT DISTINCT(A.outlook), pLoop, 'Outlook', (SELECT COUNT(*) FROM tblKolom WHERE iterasi = pLoop AND outlook = A.outlook), (SELECT COUNT(*) FROM tblKolom WHERE iterasi = pLoop AND outlook = A.outlook AND play = 'No'), (SELECT COUNT(*) FROM tblKolom WHERE iterasi = pLoop AND outlook = A.outlook AND play = 'Yes') FROM tblKolom AS A WHERE iterasi = pLoop;
...
BEGIN
	...
	SELECT 'AHH MANTABB , AKHIRNYA SELESAI' AS 'C-45', 'JUJUR SA SU BILANG SA' AS 'DE YANG MATI GILA SA';
